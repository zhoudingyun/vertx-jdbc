package com.cloud.vertx.jdbc.util;
...
public final class ArrayListUtils {
     ...
     * @return 格式化后的新列表。
     */
    public static List<String> map(Collection<?> from, String format, Map<String, String> funcs) {
        List<String> to = new ArrayList<>(from.size());
        Set<String> keys;
        for (Object e : from) {
            if ((funcs != null)) {
                keys = funcs.keySet();
                if (keys.contains(e))
                    to.add(String.format(funcs.get(e), e));
                else
                    to.add(String.format(format, e));
            } else
                to.add(String.format(format, e));
